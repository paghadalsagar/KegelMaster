...
class BasicBarChart: UIView {
    ...
    private let presenter = BasicBarChartPresenter(barWidth: 30, space: 10)
    ...
    private func showEntry(index: Int, entry: BasicBarEntry, animated: Bool, oldEntry: BasicBarEntry?) {
        ...
        mainLayer.addTextLayer(frame: entry.textValueFrame, color: UIColor.black.cgColor, fontSize: 12, text: entry.data.textValue, animated: animated, oldFrame: oldEntry?.textValueFrame)

        // Show a title below the bar
        mainLayer.addTextLayer(frame: entry.bottomTitleFrame, color: UIColor.black.cgColor, fontSize: 12, text: entry.data.title, animated: animated, oldFrame: oldEntry?.bottomTitleFrame)
    }
    ...
    private func showHorizontalLines() {
        ...
        lines.forEach { (line) in
            mainLayer.addLineLayer(lineSegment: line.segment, color: #colorLiteral(red: 0.2549019754, green: 0.2745098174, blue: 0.3019607961, alpha: 1).cgColor, width: line.width, isDashed: line.isDashed, animated: false, oldSegment: nil)
        }
    }
